...
def calculate_term_frequencies(tokens):
    return dict(Counter(tokens))def build_in
verted_index(crawled_data):
...
def main():
    ...
    if not crawled_data:
        print("No crawled data found. Run crawler.py first.")
        return
    
    print(f"Building index from {len(crawled_data)} documents...")
    
    index_data = build_inverted_index(crawled_data)
    save_json_data(index_data, 'data/inverted_index.json')
